...
const Topbar: React.FC<Props> = ({ onOpenSidebar }) => {
  ...
  return (
    <header className="h-16 sticky top-0 z-30 bg-gray-900/70 backdrop-blur-xl border-b border-gray-800 flex items-center px-3 md:px-6">
      {/* Mobile: open sidebar */}
      <button
        className="md:hidden mr-2 p-2 rounded-lg text-gray-300 hover:text-white hover:bg-gray-800"
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={logout}
          className="hidden sm:inline-flex items-center px-3 py-1.5 text-sm rounded-xl bg-gray-800 border border-gray-700 text-gray-100 hover:bg-gray-700"
        >
          Logout
        </button>
        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-gray-600 to-gray-800 border border-gray-700 flex items-center justify-center text-gray-100 font-semibold shadow">
          U
        </div>
      </div>
    </header>
